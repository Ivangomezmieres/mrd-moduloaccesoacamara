...
type CorrelacionEstado = 'OK' | 'Hueco' | 'Aislado';

interface CorrelacionInfo {
  estado: CorrelacionEstado;
  faltantes?: number[];
}
...
const calcularCorrelacion = (documents: DocumentRecord[]): { 
  ...
  const todosLosFaltantes: number[] = [];
  let totalHuecos = 0;

  docsConNumero.forEach((doc, index) => {
    if (index === 0) {
      correlacionPorId[doc.id] = { estado: 'Aislado' };
      return;
    }

    const anterior = docsConNumero[index - 1];
    const diff = doc.numero - anterior.numero;

    if (diff === 1) {
      correlacionPorId[doc.id] = { estado: 'OK' };
    } else if (diff > 1 && diff <= UMBRAL_DIFERENCIA) {
      const faltantes: number[] = [];
      for (let i = anterior.numero + 1; i < doc.numero; i++) {
        faltantes.push(i);
        todosLosFaltantes.push(i);
      }
      correlacionPorId[doc.id] = { estado: 'Hueco', faltantes };
      totalHuecos++;
    } else {
      correlacionPorId[doc.id] = { estado: 'Aislado' };
    }
...
const SuperAdminDashboard = () => {
  ...
  const { correlacionPorId, todosLosFaltantes } = useMemo(() => {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <Dialog open={showHuecosModal} onOpenChange={setShowHuecosModal}>
        <DialogContent className="max-w-md">
          ...
          <div className="py-4">
            {todosLosFaltantes.length > 0 ? (
              <div>
                <p className="text-sm text-muted-foreground mb-2">
                  Se han detectado {todosLosFaltantes.length} partes que faltan:
                </p>
              ...
              <p className="text-center text-muted-foreground">
                No se han detectado partes que faltan.
              </p>
            )}
          </div>
        </DialogContent>
